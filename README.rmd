...
summary(scot)
```
Some data type will be changed.
...
The design is a randonmized blocked design
...
```{R}
summaryBy(tot_vol~treatment, data = scot, FUN = sum)
```
...
```{r}
## Post hoc test
summary(TukeyC(s_paiha, which = 'treatment'))
```

**Diameter at breastheight**
```{r}
## Post hoc test
summary(TukeyC(s_dbh, which = 'treatment'))
```
