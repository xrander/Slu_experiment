...
**Data Exploration**
Package used is the doBy library doBy, dplyr, lattice, ggplot, TukeyC
To install the package run the commandlike this `install.packages('doBy')` as an example.
...
The first data used is the 'popdata' which is accessible at [popdata.txt](https://raw.githubusercontent.com/xrander/SLU-Plantation-Experimentation/master/Data/popdata.txt). It is an experiment to see the performance of clones. This is a part of a much larger experiment and only a part is used for the first analysis.
...
Given the figures, we need extra information to be able to estimate the density of the stand.
To do that, we estimate the plot density then extrapolate to a hectare
````{r}
